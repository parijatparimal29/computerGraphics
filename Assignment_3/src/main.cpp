...
bool Sphere::intersect(const Ray &ray, Intersection &hit) {
    ...
    h = sqrt(h);
	hit.position = Vector3d(-b-h, -b+h,h);
	hit.normal = oc.normalized();
	hit.ray_param = -b+h;
	return true;
}
...
bool Parallelogram::intersect(const Ray &ray, Intersection &hit) {
	...
	UVt << -u(0),  -v(0), ray_direction(0), 
			-u(1),  -(1), ray_direction(1), 
			-u(2),  -v(2), ray_direction(2);
	AE << (origin - ray.origin);
	...
	if (x(0)>=0 && x(1)>=0 && x(2)>0 && x(0)<=1 && x(1)<=1){
		...
		Vector3d ray_normal = (u.cross(v)); //||u x v|| or reverse whichever's negative
		hit.position = ray_intersection;
		hit.normal = ray_normal;
		hit.ray_param = x(2);
		return true;
	}
	return false;
...
Vector3d ray_color(const Scene &scene, const Ray &ray, const Object &obj, const Intersection &hit, int max_bounce) {
	...
	for (const Light &light : scene.lights) {
		...
		// TODO: done Shoot a shadow ray to determine if the light should affect the intersection point
		Ray shadow_ray;
	...
	Vector3d reflection_color = mat.reflection_color;
...
bool is_light_visible(const Scene &scene, const Ray &ray, const Light &light) {
	...
	for(int i=0;i<scene.objects.size();i++){
		Intersection hit = Intersection();
		if(scene.objects[i]->intersect(ray, hit)){
			if(hit.position == light.position)
				return false;
		}
	}
	return true;
